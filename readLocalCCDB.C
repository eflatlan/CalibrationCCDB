...
void readLocalCCDB(long ts = 9999999999000, const char* ccdb = "localhost:8080")
{
  ...
  int cnt = 0;
	for (TF1& tf1 : *mChargeCut) {
		std::unique_ptr<TCanvas> cChargeCut = std::make_unique<TCanvas>(Form("LOCAL_ChargeCut number %i", cnt), Form("LOCAL_ChargeCut number %i", cnt), 1200, 400);
		tf1.Draw();
